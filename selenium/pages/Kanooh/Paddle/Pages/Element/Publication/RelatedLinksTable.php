...
/**
 * @file
 * Contains \Kanooh\Paddle\Pages\Element\Publication\RelatedLinksTable.
 */
 ...
 * List of documents.
 *
 * @property RelatedLinksTableRow[] $rows
 * @property \PHPUnit_Extensions_Selenium2TestCase_Element $addAnotherItem
 */
class RelatedLinksTable extends Table
{
    /**
    ...
    public function __get($name)
    {
        switch ($name) {
            case 'rows':
                ...
                $items = array();
                foreach ($rows as $row) {
                    $items[] = new RelatedLinksTableRow($this->webdriver, $row);
                }

                return $items;
                break;
            case 'addAnotherItem':
                return $this->webdriver->byName('field_paddle_kce_related_links_add_more');
                break;
        }
