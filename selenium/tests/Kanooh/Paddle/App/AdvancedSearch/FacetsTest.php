...
/**
 * @file
 * Contains \Kanooh\Paddle\App\AdvancedSearch\FacetsTest.
 */
...
/**
 * Tests the facets behaviour for the Advanced search paddlet.
 *
 * @runTestsInSeparateProcesses
 * @preserveGlobalState disabled
 */
class FacetsTest extends AdvancedSearchTestBase
{
    /**
    ...
    public function testVocabularyTermsFilterWhenEnabled()
    {
        ...
        $this->publishPage($this->contentCreationService->createLandingPage(null, $landing_page_title));
        $overview_page_title = $this->alphanumericTestDataProvider->getValidValue();
        $this->publishPage($this->contentCreationService->createOverviewPage($overview_page_title));
        ...
        $this->nodeEditPage->go($nid);
        $this->nodeEditPage->advancedSearchForm->vocabularyTermsTable->rows[$terms[2]['#tid']]->enabled->uncheck();
        $this->nodeEditPage->contextualToolbar->buttonSave->click();
        $this->adminNodeViewPage->checkArrival();
    ...
    public function testMultipleTermsTaggedNodeFilter()
    {
        ...
        $this->nodeEditPage->go($nid);
        $this->nodeEditPage->advancedSearchForm->vocabularyTermsTable->rows[$terms[1]['#tid']]->enabled->uncheck();
        $this->nodeEditPage->advancedSearchForm->vocabularyTermsTable->rows[$terms[2]['#tid']]->enabled->check();
