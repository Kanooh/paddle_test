...
<div id="page" class="full-height content-background-canvas">
  ...
  <?php endif; ?>
  <header>
    <div class="max-width header-background-canvas">
        ...
        <div class="row hide-overflow">
          <div class="col-xs-12">
            <nav id="main-nav" class="clearfix">
              <?php if ($page['header_bottom']):?><?php print render($page['header_bottom']); ?><?php endif; ?>
            </nav>
            <?php if (!empty($breadcrumb) && $show_breadcrumb): ?><div id="breadcrumb"><?php print $breadcrumb; ?></div><?php endif; ?>
            <?php print render($title_prefix); ?>
            <?php if ($title): ?><h1 class="title" id="page-title"><?php print $title; ?></h1><?php endif; ?>
            <?php print render($title_suffix); ?>
            <?php if ($page['content_sub_top']):?><?php print render($page['content_sub_top']); ?><?php endif; ?>
            <?php if ($messages): ?>
              <div id="messages">
              ...
              <!-- /.section, /#messages -->
            <?php endif; ?>
            <?php if ($page['featured']): ?><div id="featured"><div class="section clearfix"><?php print render($page['featured']); ?></div></div> <!-- /.section, /#featured --><?php endif; ?>
            ...
            <div class="container-fluid">
              <?php if (!empty($search_page_logo)): ?>
                <div class="search-page-logo"><?php print render($search_page_logo); ?></div>
              <?php endif; ?>
              <div class="row hide-overflow">
                <?php if ($page['sidebar_first']): ?>
                  <div class="col-sm-3 culturefeed-facets"><?php print render($page['sidebar_first']); ?></div>
                <?php endif; ?>

                <?php if ($page['content_top']) :?><div class="col-sm-8 col-md-9"><?php print render($page['content_top']); ?></div><?php endif; ?>
                <?php if ($page['content']): ?><div class="col-sm-8 col-md-9" id="page-content"><?php print render($page['content']); ?></div><?php endif; ?>
              </div>
            </div>
          </div>
          <?php if (!empty($top_pane_tag_results)): ?>
            <div class="agenda-search-top-pane pane-content pane-uitdatabank-pane spotlight">
            <?php if (!empty($top_pane_title)): ?>
              <div class="agenda-search-pane-title pane-section-top">
              <?php if (variable_get('paddle_style_display_pane_top_as_h2', FALSE)): ?>
                <h2><?php print $top_pane_title; ?></h2>
              <?php else: ?>
                <?php print $top_pane_title; ?>
              <?php endif; ?>
              </div>
            <?php endif; ?>
            <div class="paddle-cultuurnet-spotlight-events pane-section-body row">
              <?php foreach ($top_pane_tag_results as $event) :?>
                <div class="paddle-cultuurnet-spotlight-event col-md-3 col-sm-6">
                  <?php if (!empty($event['title'])) : ?>
                    ...
                    <div class="paddle-cultuurnet-spotlight-details">
                      <?php if (!empty($event['image_url'])) : ?>
                        <div role="img" aria-label="<?php print $event['title']; ?>" title="<?php print $event['title']; ?>" class="spotlight-image" style="background-image:url(<?php print $event['image_url']; ?>)"></div>
                      <?php endif; ?>
                      <div class="spotlight-bottom">
                        <h3><?php print $event['title']; ?></h3>
                        <?php if (!empty($event['period'])) : ?>
                          <div class="spotlight-period">
                            <?php print $event['period']; ?>
                          </div>
                        <?php endif; ?>
                      </div>
                    </div>
          ...
          <?php endif; ?>
          <?php if (!empty($bottom_pane_tag_results)) : ?>
            <div class="agenda-search-bottom-pane pane-content pane-uitdatabank-pane spotlight">
            <?php if (!empty($bottom_pane_title)): ?>
              <div class="agenda-search-pane-title pane-section-top">
                <?php if (variable_get('paddle_style_display_pane_top_as_h2', FALSE)): ?>
                  <h2><?php print $bottom_pane_title; ?></h2>
                <?php else: ?>
                  <?php print $bottom_pane_title; ?>
                <?php endif; ?>
              </div>
            <?php endif; ?>
            <div class="paddle-cultuurnet-spotlight-events pane-section-body row">
              <?php foreach ($bottom_pane_tag_results as $event) :?>
                <div class="paddle-cultuurnet-spotlight-event col-md-3 col-sm-6">
                  <?php if (!empty($event['title'])) : ?>
                    <?php if (!empty($event['url'])) : ?>
                      <a href="<?php print $event['url']; ?>" class="paddle-cultuurnet-event-link">
