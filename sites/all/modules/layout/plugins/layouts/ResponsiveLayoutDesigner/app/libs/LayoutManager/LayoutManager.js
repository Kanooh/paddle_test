(function (RLD, $) {
  ...
  RLD['LayoutManager'] = (function build() {
    ...
    LayoutManager.prototype.addRegionHandler = function (event) {
      ...
      .on('change', function() {
        if (this.value !== "null"){
          $('.ui-dialog-buttonset button.save')[0].disabled = false;
          $('.rld-dialog input')[0].disabled = true;
        }
        else {
          $('.rld-dialog input')[0].disabled = false;
        }
      });
      ...
      var machineNamePrint = $.proxy(function machineNamePrintProxy(checker, $input, $display, event) {
        ...
        if (isUnique ) {
          // don't allow empty system names.
          if (machine_name.length === 0) {
            $display
              .text('')
            $('.ui-dialog-buttonset button.save')[0].disabled = true;
            $(".rld-dialog select[name='available-region-select']")[0].disabled = false;
            $(".rld-dialog select[name='available-region-select']").css({"background-color":"rgb(255, 255, 255)"});
          }
          else {
            $display
              .text(machine_name)
            ...
            $('.ui-dialog-buttonset button.save')[0].disabled = false;
            $(".rld-dialog select[name='available-region-select']")[0].disabled = true;
            $(".rld-dialog select[name='available-region-select']").css({"background-color":"rgb(235, 235, 228)"});
          }
        }
        else {
          $display
          .text('"' + machine_name + '" is already in use.')
          .css({
            'color': 'red'
          });
          // Disable Save button.
    ...
    LayoutManager.prototype.requestRegionAdd = function (location, $dialog, event) {
      ...
      var region;
      var existingRegionsSelect = $dialog.find('[name="available-region-select"]');
      var $selectedAvailableRegion = existingRegionsSelect.find('option').filter(':selected');
      if ($selectedAvailableRegion.length > 0 && $selectedAvailableRegion.val() !== 'null') {
        region = this.availableRegionList.getItem($selectedAvailableRegion.val());
        ...
        // if the manual entered value is in the list of existing regions, remove it from that list.
        var regionExists = existingRegionsSelect.find('option[value="' + this.candidateRegionMachineName + '"]').val();
        if (regionExists !== undefined) {
          region = this.availableRegionList.getItem(regionExists);
          this.availableRegionList.removeItem(region);
        }
      }
