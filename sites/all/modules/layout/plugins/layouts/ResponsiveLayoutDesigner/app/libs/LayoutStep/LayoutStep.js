(function (RLD, $) {
  ...
  RLD['LayoutStep'] = (function () {
    ...
    LayoutStep.prototype.startRegionResize = function (event) {
      ...
      // The UI size has a max width of 1024 pixels so we need to deal with
      // breakpoints which are bigger than 1024 pixels as well.
      if (1024 > this.step.info('size')) {
        data.frame = Math.floor(Number(this.step.info('size')) / data.totalColumns);
      }
      else {
        data.frame = Math.floor(1024 / data.totalColumns);
      }
      ...
      // We calculate the span width of the active region, the span width
      // occupied by regions before the active region and the span width
      // of the regions behind the active region to calculate how much we can
      // move the the left or the right.
      var activeRegionFound = false;
      var beforeSpan = 0;
      var behindSpan = 0;
      var activeRegionSpan = 0;
      Array.from(data.units).forEach(function (region) {
        // We just want to retrieve the units of the 'region' type.
        if (region.type === 'region') {
          if (!activeRegionFound) {
            if (region.active === true) {
              activeRegionFound = true;
              activeRegionSpan = region.span;
            }
            else {
              beforeSpan += region.span;
            }
          }
          else {
            behindSpan += region.span;
          }
        }
      });
      // Calculate the left and right bounds for the resizing.
      if (data.side === 'left') {
        data.rightMaxTraversal = data.totalColumns - beforeSpan - behindSpan - 1;

        if (beforeSpan > 0) {
          data.leftMaxTraversal =  (data.totalColumns - behindSpan - activeRegionSpan - 1) * -1;
        }
        else {
          data.leftMaxTraversal = 0;
        }
      }
      else {
        if (behindSpan > 0) {
          data.rightMaxTraversal = data.totalColumns - beforeSpan - activeRegionSpan - 1;
        }
        else {
          data.rightMaxTraversal = 0;
        }
        data.leftMaxTraversal =  (data.totalColumns - behindSpan - beforeSpan - 1) * -1;
      }
