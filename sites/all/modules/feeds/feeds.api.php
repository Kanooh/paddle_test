...
/**
 * @defgroup parse Parser-related hooks
 * @{
 */

/**
 * Invoked after a feed item is parsed by the Common Syndication parser.
 *
 * @param array $item
 *   Set of parsed values.
 * @param SimpleXMLElement $rss_item
 *   Non-interpreted, parsed XML structure of the actually parsed item.
 * @param SimpleXMLElement $feed
 *   Non-interpreted, parsed XML structure of the entire feed.
 */
function hook_common_syndication_parser_item_alter(array &$item, SimpleXMLElement $rss_item, SimpleXMLElement $feed) {
  // For example, parse date in a smarter way for RSS2.0 feeds:
  if (isset($rss_item['pubDate'])) {
    $item['timestamp'] = _my_module_date_convert($rss_item['pubDate']);
  }
}
